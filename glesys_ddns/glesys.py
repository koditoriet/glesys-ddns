...
class Record:
    ...
    def from_dict(_dict: dict[str, Any]) -> "Record":
        filtered_fields = {
            k: v for k, v in _dict.items() if k in Record.__dataclass_fields__
        }
...
class GleSYS:
    ...
    def __init__(
        self, api_user: str, api_key: str, api_url: str = _DEFAULT_API_URL
    ) -> None:
    ...
    def _post(self, path: str, json: Any) -> requests.Response:
        ...
        if not resp.ok:
            raise GleSYSError(
                f"GleSYS POST request to {path} failed with status code {resp.status_code}"
            )
        return resp
    ...
    def list_records(self, domain: str) -> list[Record]:
        ...
        return [
            Record.from_dict(record) for record in resp.json()["response"]["records"]
        ]
